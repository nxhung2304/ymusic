...
## Metadata
- **Title:** YouTubeDatasource: search(query) → List<Song>
...
Tạo `YouTubeDatasource` — shared service đặt trong `lib/core/services/`, gọi `youtube_explode_dart` để search bài nhạc và parse kết quả thành `List<SongModel>`.
...
- [ ] Tạo `lib/core/services/youtube_service.dart`
  - Class `YouTubeDatasource` với `YoutubeExplode` inject vào constructor
